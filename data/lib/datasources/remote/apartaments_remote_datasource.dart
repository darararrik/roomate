...
class ApartamentsRemoteDataSource implements ApartamentsDataSource {
  ...
  @override
  Future<Either<RemoteException, List<ApartamentPreviewModel>>> fetchApartaments(ApartamentFilterModel filter) async {
    ...
    return result.fold(
      (result) => Left(result),
      (result) => Right(result.items.map(ApartamentMapper.toPreviewModel).toList()),
    );
  }

  @override
  Future<Either<RemoteException, ApartamentModel>> fetchApartamentById(String id) async {
    final result = await _client.get<ApartamentData>(
      ApiUrlConstants.adsId(id),
      transformer: (json) => ApartamentData.fromJson(json),
    );
    return result.fold((result) => Left(result), (result) => Right(ApartamentMapper.toModel(result)));
  }
  ...
  @override
  Future<Either<RemoteException, void>> createAd(CreateAdFormRequestData request) async {
  ...
  @override
  Future<Either<RemoteException, AdApplicationSubmitModel>> applyToAd(String adId) async {
    final result = await _client.post<AdApplicationSubmitData>(
      ApiUrlConstants.adsApply(adId),
      needAuth: true,
      transformer: (json) => AdApplicationSubmitData.fromJson(json as Map<String, dynamic>),
    );

    return result.fold((error) => Left(error), (data) => Right(AdApplicationMapper.toSubmitModel(data)));
  }
  ...
  Future<Either<RemoteException, List<MyAdModel>>> fetchMyAds() async {
    final result = await _client.get<List<MyAdData>>(
      ...
      needAuth: true,
      transformer: (json) => _parseList(json, itemParser: (item) => MyAdData.fromJson(item as Map<String, dynamic>)),
    );

    return result.fold((error) => Left(error), (items) => Right(items.map(OwnerAdsMapper.toMyAdModel).toList()));
  }

  @override
  Future<Either<RemoteException, List<AdApplicationModel>>> fetchIncomingAdApplications({String? status}) async {
    final result = await _client.get<List<AdApplicationData>>(
      ApiUrlConstants.myAdApplications,
      needAuth: true,
      query: status == null ? null : {'status': status},
      transformer: (json) =>
          _parseList(json, itemParser: (item) => AdApplicationData.fromJson(item as Map<String, dynamic>)),
    );

    return result.fold((error) => Left(error), (items) => Right(items.map(OwnerAdsMapper.toApplicationModel).toList()));
  }

  @override
  Future<Either<RemoteException, AdApplicationDetailModel>> fetchIncomingAdApplicationDetails(
    ...
    return result.fold((error) => Left(error), (item) => Right(OwnerAdsMapper.toApplicationDetailModel(item)));
  }

  @override
  Future<Either<RemoteException, void>> rejectIncomingAdApplication(String applicationId) async {
  ...
  @override
  Future<Either<RemoteException, void>> acceptIncomingAdApplication(String applicationId) async {
    return _client.post<void>(
  ...
  List<T> _parseList<T>(dynamic json, {required T Function(dynamic item) itemParser}) {
    final rawItems = switch (json) {
      ...
      Map<String, dynamic> map when map['ads'] is List<dynamic> => map['ads'] as List<dynamic>,
  ...
  AdApplicationData _parseApplication(dynamic json) {
    if (json is Map<String, dynamic>) {
      if (json['application'] is Map<String, dynamic>) {
        return AdApplicationData.fromJson(json['application'] as Map<String, dynamic>);
      }
