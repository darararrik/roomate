...
class GroupsRemoteDataSource implements GroupsDataSource {
  GroupsRemoteDataSource(this._client, this._tokenService);
  ...
  Future<Either<RemoteException, List<GroupListItemModel>>> fetchGroups({
    ...
    return result.fold(
      ...
      (data) => Right(
        data.items
            .map(
              (item) => GroupMapper.toListItemModel(
                item,
  ...
  fetchIncomingGroupApplications({String? status}) async {
    final result = await _client.get<List<IncomingGroupApplicationData>>(
      ...
      transformer: (json) => _parseList(
        json,
        itemParser: (item) =>
            IncomingGroupApplicationData.fromJson(item as Map<String, dynamic>),
      ),
    );
    ...
    return result.fold(
      ...
      (items) => Right(
        items
            .map(
              (item) => GroupMapper.toIncomingApplicationModel(
                item,
                baseUrl: _client.publicBaseUrl,
              ),
            )
  ...
  fetchIncomingGroupApplicationDetails(String applicationId) async {
    ...
    return result.fold(
      (error) => Left(error),
      (item) => Right(
        GroupMapper.toIncomingApplicationDetailModel(
          item,
          baseUrl: _client.publicBaseUrl,
        ),
      ),
  ...
  IncomingGroupApplicationData _parseApplication(dynamic json) {
    if (json is Map<String, dynamic>) {
      if (json['application'] is Map<String, dynamic>) {
        return IncomingGroupApplicationData.fromJson(
