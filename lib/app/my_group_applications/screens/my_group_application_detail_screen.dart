...
import 'package:roomate/widgets/cards/group_application_group_card.dart';

@RoutePage()
class MyGroupApplicationDetailScreen extends ConsumerWidget {
  const MyGroupApplicationDetailScreen({
    super.key,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final notifier = ref.read(
      myGroupApplicationDetailProvider(applicationId).notifier,
    );
    final isSubmitting = ref.watch(incomingGroupApplicationActionProvider);
              ...
              SliverPadding(
                ...
                sliver: SliverList(
                  delegate: SliverChildListDelegate([
                    ...
                    GroupApplicationGroupCard(
                      group: application.group,
                      onTap: () => context.pushRoute(
                        AboutGroupRoute(groupId: application.group.id),
                      ),
                    ),
                    const SizedBox(height: S.p16),
                    Text(
  ...
  Future<void> _handleAccept(BuildContext context, WidgetRef ref) async {
    final error = await ref
        .read(incomingGroupApplicationActionProvider.notifier)
