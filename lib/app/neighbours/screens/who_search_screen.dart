...
import 'package:roomate/app/neighbours/notifier/who_search_filter_notifier.dart';
...
class WhoSearchScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final minGroupSizeController = useTextEditingController(
      text: filter.minGroupSize?.toString() ?? '',
    );
    final maxGroupSizeController = useTextEditingController(
      text: filter.maxGroupSize?.toString() ?? '',
    );
    ...
    final genderOptions = [
      OptionModel(id: 1, title: l10n.whoSearchGenderAny),
      OptionModel(id: 2, title: l10n.whoSearchGenderMen),
      OptionModel(id: 3, title: l10n.whoSearchGenderWomen),
      OptionModel(id: 4, title: l10n.whoSearchGenderOther),
    ];

    useEffect(() {
      _syncControllerText(
        minGroupSizeController,
        filter.minGroupSize?.toString() ?? '',
      );
      _syncControllerText(
        maxGroupSizeController,
        filter.maxGroupSize?.toString() ?? '',
      );
      _syncControllerText(considerationController, filter.searchQuery);
      return null;
    }, [filter.minGroupSize, filter.maxGroupSize, filter.searchQuery]);
              ...
              sliver: SliverList.list(
                children: [
                  ...
                  ChipWrap(
                    ...
                    selectedIds: {_genderToOptionId(filter.gender)},
                    singleSelection: true,
                    onSelectionChanged: (ids) {
                      final selectedId = ids.isEmpty ? 1 : ids.first;
