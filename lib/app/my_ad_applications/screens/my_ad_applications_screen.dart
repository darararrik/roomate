...
import 'package:roomate/widgets/cards/application_card.dart';

@RoutePage()
class ApplicationsTabViewScreen extends ConsumerWidget {
  const ApplicationsTabViewScreen({super.key});
                    ...
                    delegate: _PinnedHeaderDelegate(
                      ...
                      child: ColoredBox(
                        ...
                        child: _ApplicationsTabs(
                          ...
                          newCount: _watchOwnerApplicationsCount(
                            ref,
                            _newStatuses,
                          ),
                          archivedCount: _watchOwnerApplicationsCount(
                            ref,
                            _archivedStatuses,
                          ),
                        ),
...
@RoutePage()
class NewApplicationsScreen extends StatelessWidget {
  const NewApplicationsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return _ApplicationsTabContent<AdApplicationModel>(
      statuses: ApplicationsTabViewScreen._newStatuses,
      watchItems: _watchOwnerApplications,
      itemBuilder: _buildOwnerApplicationItem,
...
class ArchivedApplicationsScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const _ApplicationsTabContent<AdApplicationModel>(
      statuses: ApplicationsTabViewScreen._archivedStatuses,
      watchItems: _watchOwnerApplications,
      itemBuilder: _buildOwnerApplicationItem,
    );
  }
...
int _watchOwnerApplicationsCount(
  WidgetRef ref,
  ...
  for (final status in statuses) {
    count +=
        ref
            .watch(applicationsForAdsByStatusProvider(status))
            .asData
            ?.value
            .length ??
        0;
...
AsyncValue<List<AdApplicationModel>> _watchOwnerApplications(
  WidgetRef ref,
  List<AdApplicationStatus> statuses,
) {
  final states = [
    for (final status in statuses)
      ref.watch(applicationsForAdsByStatusProvider(status)),
  ];

  return _combineApplicationsStates(states);
}

Widget _buildOwnerApplicationItem(
  BuildContext context,
  AdApplicationModel item,
) {
  return ApplicationCard(
    item: item,
    onTap: () =>
        context.pushRoute(MyAdApplicationDetailRoute(applicationId: item.id)),
  );
}
